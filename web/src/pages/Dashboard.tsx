...
export default function Dashboard(){
  const { data, isLoading } = useQuery(['events'], () => listUpcomingEvents())
    ...
    <div className="grid grid-cols-12 gap-4">
      <div className="col-span-4">
        <Card title="Upcoming (90 days)">
          ...
          <div className="mt-3"><Button onClick={() => fetch((import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001') + '/api/calendar/sync', { method: 'POST' }).then(r=>r.json()).then(()=>window.location.reload())}>Sync now</Button></div>
        </Card>
      </div>
